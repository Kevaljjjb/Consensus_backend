-- Dashboard overview read-path optimization.
-- Idempotent migration: ensures numeric financial columns, parser trigger,
-- backfill, and indexes needed by /api/dashboard/overview.
...
-- Safe backfill from text columns. Malformed values resolve to NULL.
UPDATE raw_listings
SET
...
WHERE
    price_num IS DISTINCT FROM parse_financial_numeric(price)
    OR gross_revenue_num IS DISTINCT FROM parse_financial_numeric(gross_revenue)
    OR cash_flow_num IS DISTINCT FROM parse_financial_numeric(cash_flow)
    OR ebitda_num IS DISTINCT FROM parse_financial_numeric(ebitda);
